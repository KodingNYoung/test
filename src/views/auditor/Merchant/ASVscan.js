...
import { getASVRequest, uploadASV } from 'store/actions/auditorActions';
...
const ASVscan = (props) => {
  ...
  const { uploadASV, getASVRequest, merchant_asvscans } = props;

  useEffect(() => {
    const getMerchantScan = async () => {
      setLoadingContent(true);
      const res = await getASVRequest();
  ...
  const handleSubmit = async () => {
    ...
    const token = await uploadASV(id, formdata);
    if (token.status === 'success') {
      setAlert(true);
      setMessage(token.message);
      setLoading(false);
      let scan = merchant_asvscans.filter((item) => item.id === id);
...
const mapStateToProps = (state) => {
  return {
    merchant_asvscans: state?.auditorReducers?.merchant_asvscans,
  };
};
